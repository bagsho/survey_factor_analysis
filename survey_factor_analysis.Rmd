...
table<-table %>% pivot_longer(!measure, names_to = "factor", values_to = "loading") %>% group_by(measure) %>% filter(loading==max(loading)) %>% select(factor,measure,loading) %>% arrange(factor,desc(loading))
table
