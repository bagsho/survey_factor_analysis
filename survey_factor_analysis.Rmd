...
results<-factanal(data2, 4, rotation="varimax")
table<-results$loadings[,1:4] %>% as_tibble() %>% mutate(measure=results$loadings[,1:4] %>% row.names) %>%  select(measure, everything())
table %>% write_excel_csv("results.csv")
```
